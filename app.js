...
/*// Ruta para recibir el UPC code
app.post('/api/item', async (req, res) => {
    try {
        console.log('POST /api/item accessed'); // Log de acceso a la ruta
        ...
        res.json({
            success: true,
            message: "UPC code received successfully",
            data: { upccode }
        });
    } catch (error) {
        console.error('Server Error:', error.message); // Log detallado del error
        res.status(500).json({
            success: false,
